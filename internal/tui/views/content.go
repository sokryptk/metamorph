...
func (c Content) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		cols := c.cont.Columns()
		for i := range cols {
			cols[i].Width = int(math.Floor(float64(msg.Width)/float64(len(cols)))) - 2
		}

		c.cont.SetHeight(msg.Height)
		c.cont.SetWidth(msg.Width)
		c.h = msg.Height
		c.cont.SetColumns(cols)
	case tea.KeyMsg:
		slog.Info("Key pressed", "key", msg.String())
	}

	c.cont, cmd = c.cont.Update(msg)
	return c, cmd
}

func (c Content) View() string {
	box := lipgloss.NewStyle().Height(c.h)

	return box.Render(c.cont.View())
}
